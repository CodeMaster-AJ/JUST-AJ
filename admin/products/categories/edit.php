...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('SELECT id FROM product_categories WHERE slug = ? AND id != ?');
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare('UPDATE product_categories SET name = ?, slug = ?, description = ?, sort_order = ?, status = ? WHERE id = ?');
